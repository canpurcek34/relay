...
This guide is for people who want to use more than one Claude setup in Relay. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Relay](./install.md).
...
In Relay Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Relay uses Claude Code's normal config directory.

When you set this field, Relay points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Relay looks.

Then add another Claude provider in Relay:
...
Relay only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Relay keeps separate config directories isolated
...
Add or edit a Claude provider in Relay Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Relay stores the value as a server secret and does not
...
Relay does not need a special Claude Code Router provider. Treat the router as a Claude
