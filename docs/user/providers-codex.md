...
This guide is for people who want to use more than one Codex account in Relay. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Relay](./install.md).
...
In Relay Settings, name it something obvious:
...
In Relay Settings, add another Codex provider:
...
Relay shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Relay treats it
